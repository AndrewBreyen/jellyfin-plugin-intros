...
namespace Jellyfin.Plugin.Intros {
  public class IntroManager {
    ...
    private readonly int[] _intros = {
        459725398, 440978154, 440793415, 440978850, 462141918, 456990827,
        442744420, 459723868, 464974136, 464989433, 443076423, 443381251,
        442179153, 443404430, 443082153, 443076524, 443076432, 443081703,
        443076518, 441615803, 443076410, 443081835, 443081756, 443404417,
        443081683, 443076427, 443404395, 443081932, 440785270, 440791181,
        443125346, 443076495, 445012069, 443082249, 465047881, 443381171,
        443381220, 465120319, 464496810, 482322121, 465061336, 465017117,
        443381155, 482679433, 443081704, 443380983, 465018157, 443404335,
        443076479, 443404299, 443381025, 443081757, 443380999, 443404077,
        463022111, 442981865, 449256975, 443404049, 443510978, 448034833,
        444952001, 443081763, 443404007, 443076414, 465473327, 443403981,
        443082063, 483716521, 484071435, 483181060, 465031184, 443403947,
        443132003, 443082060, 485178920, 465002705, 443081913, 443076389,
        482670089, 483691215, 443076530, 464967809};

    private readonly string _cache =
        Plugin.ApplicationPaths.CachePath + "/intros/";
    ...
    public IEnumerable<IntroInfo> Get() {
      ...
      var path = Intro(Plugin.Instance.Configuration.Intro,
                       Plugin.Instance.Configuration.Resolution);
      ...
      yield return new IntroInfo{ItemId = Plugin.Instance.Configuration.Id,
                                 Path = path};
    }
    ...
    private void Cache(int intro) {
      ...
      if (match.Groups.Count != 2) return;
        ...
        var difference =
            Math.Abs(stream.height - Plugin.Instance.Configuration.Resolution);
    ...
    private void UpdateLibrary(string title, string path) {
      var result = Plugin.LibraryManager.GetItemsResult(new InternalItemsQuery{
          HasAnyProviderId =
              new Dictionary<string, string>{{"prerolls.video", title}}});
      ...
      var video = new Video{
          Id = Guid.NewGuid(), Path = path,
          ProviderIds =
              new Dictionary<string, string>{{"prerolls.video", title}},
          Name = title
                     .Replace("jellyfin", string.Empty,
                              StringComparison.InvariantCultureIgnoreCase)
                     .Replace("pre-roll", string.Empty,
                              StringComparison.InvariantCultureIgnoreCase)
                     .Trim()};
